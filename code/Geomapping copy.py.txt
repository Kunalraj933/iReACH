...
############## Plot E010029658 LSOA (example LSOA) and its 3 closest hospitals
lsoa_code = 'E01000471'
...
if selected_lsoa.empty:
    ...
    distances = [
        selected_lsoa['distance_1_km'].iloc[0],
        selected_lsoa['distance_2_km'].iloc[0],
        selected_lsoa['distance_3_km'].iloc[0]
    ]

    for i, hospital in enumerate(closest_hospitals):
    ...
    legend_patches = [mpatches.Patch(color=color, label=f"{hospital} ({distances[i]:.1f}km)") 
                     for i, (hospital, color) in enumerate([(h, hospital_colors[h]) for h in closest_hospitals])]
    legend_patches.append(mpatches.Patch(color='yellow', label='Selected LSOA'))
    legend_patches.append(mpatches.Patch(color='gold', label='LSOA Centroid'))
